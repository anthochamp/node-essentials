...
import { sha1Js } from "@ac-kit/crypto-hash";
...
export function hmacSha1Ts(
	key: Uint8Array,
	message: Uint8Array,
): Uint8Array<ArrayBuffer> {
	return hmac(sha1Js, BLOCK_SIZE_BYTES, key, message);
}

/**
 * HMAC-SHA-1 (RFC 2104), legacy interop only. Prefers the Web Crypto kernel
 ...
 * dual-kernel shape as `sha1`/`sha256` in `@ac-kit/crypto-hash`.
 */
