...
describe("Barrier", () => {
	...
	it("should decrement arrival count if wait is aborted before barrier trips", async () => {
		const barrier = new Barrier(3);
		const ac = new AbortController();

		// First participant arrives and aborts while waiting
		const abortedWaiter = barrier.wait(ac.signal);
		ac.abort();
		await expect(abortedWaiter).rejects.toThrow();

		// Subsequent arrivals should still need the full count (3), not trip at 2
		let resolvedEarly = false;
		const w1 = barrier.wait().then(() => {
			resolvedEarly = true;
		});
		const w2 = barrier.wait().then(() => {
			resolvedEarly = true;
		});
		...
		// Third arrival trips the barrier properly
		const w3 = barrier.wait();
