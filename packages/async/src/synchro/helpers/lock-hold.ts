import { VariadicArgs, fromVariadicArgs } from "@ac-kit/core";
...
export class LockHold implements AsyncDisposable {
	...
	private static getLockId(lockable: ILock): number {
		...
		return lockable[LOCK_ID_SYMBOL];
	}
	 ...
	 */
	static async tryFrom(locks: readonly ILock[]): Promise<LockHold | null>;
	static async tryFrom(...locks: ILock[]): Promise<LockHold | null>;
	static async tryFrom(...args: VariadicArgs<ILock>): Promise<LockHold | null> {
		const locks = fromVariadicArgs(args);
