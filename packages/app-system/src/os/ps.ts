...
>(options?: O): Promise<R[]> {
	...
	const commandArgs = [
		...
		"-o",
		escapeCommandArg(columns.join(",")),
	];

	let output: string;
	try {
		const result = await execAsync(`ps ${commandArgs.join(" ")}`, {
...
function buildPsFiltersArgs(filters: PsFilters): string[] {
	...
	if (filters.group?.length) {
		args.push("-G", ...filters.group.map((n) => escapeCommandArg(`${n}`)));
	}

	if (filters.pid?.length) {
		args.push("-p", ...filters.pid.map((n) => escapeCommandArg(`${n}`)));
	}

	if (filters.tty?.length) {
		args.push("-t", ...filters.tty.map(escapeCommandArg));
	}

	if (filters.user?.length) {
		args.push("-U", ...filters.user.map((n) => escapeCommandArg(`${n}`)));
	}
