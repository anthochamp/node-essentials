import { BYTES_PER_KIB, UnimplementedError, defaults } from "@ac-kit/core";
import type { TupleToUnion, UnknownRecord } from "type-fest";

import { type PsFilters, type PsResult, ps, psParseDuration } from "./ps.js";
 ...
 *
 * The POSIX implementation reads `ps`, which Windows has no equivalent of:
 * `tasklist` and WMI both report a different field set under different names,
 * so this would be a second implementation rather than a port. Until one
 * exists, the call fails loudly instead of returning a partial snapshot a
...
>(_options?: O): Promise<R[]> {
	throw new UnimplementedError(
		"getProcessesSnapshot is not supported on Windows",
	);
}
