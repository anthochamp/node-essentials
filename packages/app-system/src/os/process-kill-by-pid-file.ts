...
import { readPidFile } from "../process/pid-file.js";
import {
...
export type ProcessKillByPidFileOptions = {
	...
	/**
	 * Wait options to use after sending the kill signal. Default is `{}` (wait
	 * for termination with default options).
	 *
...
/**
 * Kill the process whose PID is stored in the PID file and optionally wait for
 * it to terminate.
 *
 * Different signals can be used to terminate the process: - 'SIGTERM'
 * (default): Politely asks the process to terminate, allowing it to perform
 * cleanup operations. - 'SIGINT': Simulates an interrupt signal (like pressing
 * Ctrl+C in a terminal). This allows the process to handle the signal and
 * perform any necessary cleanup. - 'SIGKILL': Forces the process to terminate
 * immediately. This signal cannot be caught or ignored by the process.
 *
 ...
 * process to handle its own cleanup, including removing the PID file if
 * necessary.
 *
