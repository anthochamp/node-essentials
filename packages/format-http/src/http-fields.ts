import type { Predicate } from "@ac-kit/core";
import { CaseInsensitiveMap, stringIsEqualCaseInsensitive } from "@ac-kit/core";
			...
			if (Symbol.iterator in Object(initialFields)) {
				...
				>) {
					if (Array.isArray(valueOrValues)) {
						// oxlint-disable-next-line typescript/no-misused-spread -- false positive, valueOrValues is an array of strings
				...
				for (const [name, valueOrValuesOrUndefined] of Object.entries(
					...
					if (Array.isArray(valueOrValuesOrUndefined)) {
						// oxlint-disable-next-line typescript/no-misused-spread -- false positive, valueOrValuesOrUndefined is an array of strings
