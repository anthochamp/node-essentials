...
export type TlsSocketConnectOptions = Except<tls.ConnectionOptions, "port"> & {
	signal?: AbortSignal;
};
...
export class TlsSocket extends StreamSocket<tls.TLSSocket, TlsSocketEvents> {
	...
	static connect(
		...
		return new Promise((resolve, reject) => {
			...
			const tlsSocket = new TlsSocket(tls.connect({ ...connectOptions, port }));

			const handleError = (error: Error) => {
				tlsSocket.unsubscribe("secureConnect", handleSecureConnect);
				signal?.removeEventListener("abort", handleAbort);
				reject(error);
			};
			tlsSocket.subscribe("error", handleError, { once: true });

			const handleSecureConnect = () => {
				tlsSocket.unsubscribe("error", handleError);
				signal?.removeEventListener("abort", handleAbort);
				resolve(tlsSocket);
			};
			tlsSocket.subscribe("secureConnect", handleSecureConnect, { once: true });
