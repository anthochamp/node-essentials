	...
	describe("properties", () => {
		describe("connected socket", () => {
			...
			beforeEach(async () => {
				...
				await socket.connect(serverPort, { host: serverHost });
			});
			...
			it("should report connecting status during connection", async () => {
				...
				const connectPromise = socket.connect(serverPort, { host: serverHost });
	...
	describe("methods", () => {
		...
		describe("connect", () => {
			...
			it("should reject on connection error", async () => {
				...
				await expect(socket.connect(1, { host: "0.0.0.0" })).rejects.toThrow();
			});

			it("should reject on connection to non-existent host", async () => {
				const socket = TcpSocket.from();
				await expect(
					socket.connect(serverPort, { host: "invalid.host.example.com" }),
				).rejects.toThrow();
			});
			...
			it("should handle connection refused error", async () => {
				...
				await expect(
					socket.connect(unusedPort, { host: serverHost }),
				).rejects.toThrow();
			});
			...
			it("should reject immediately if signal is already aborted", async () => {
				...
				controller.abort();
				// Synchronous early return — never connects.
				await expect(
					socket.connect(serverPort, { signal: controller.signal }),
				).rejects.toThrow();
			});

			it("should reject when signal is aborted while connecting", async () => {
				const socket = TcpSocket.from();
				const controller = new AbortController();
				// TEST-NET-1 (RFC 5737): non-routable, connect hangs indefinitely.
				const connectPromise = socket.connect(80, {
					host: "192.0.2.1",
					signal: controller.signal,
				});
				// abort() fires the "abort" event synchronously → destroy(error) →
				// nextTick error emission → handleError → reject.
				controller.abort();
	...
	describe("integration scenarios", () => {
		...
		describe("events", () => {
			...
			it("should emit lookup event when connecting to hostname", async () => {
				...
				await socket.connect(serverPort, { host: "localhost" });
				await lookupPromise;
				await socket.end();
			});
		});
