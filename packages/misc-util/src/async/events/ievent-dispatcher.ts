...
export type EventDispatcherSubscribeOptions = {
	/**
	 * If true, the listener will be automatically unsubscribed after being called
	 * once.
	 *
	 * Default is false.
	 */
	 ...
	 *
	 * Higher values are invoked before lower values. Listeners with the same
	 * priority are invoked in the order they were registered.
	 *
	 * Default is 0.
	 */
	priority?: number;
 ...
 * dispatching.
 *
 * @template T - Tuple type representing the arguments that will be passed to
 *   listener functions.
 */
export interface IEventDispatcher<T extends unknown[] = never[]> {
	 ...
	 * @returns A function that can be called to unsubscribe the listener, or null
	...
	/**
	 * Waits for the next occurrence of the event, optionally matching a
	 * predicate.
	 *
	 * This is a convenience method that subscribes to the dispatcher, waits for
	 * the next event that matches the predicate (if provided), and then
