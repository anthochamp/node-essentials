...
describe("remove", () => {
	...
	it("removes all occurrences of an object reference", () => {
		...
		const removed = removeAll(arr, obj);
	...
	it("does not remove items that are equal but not strictly equal (objects)", () => {
		const arr = [{ a: 1 }, { a: 1 }];
		const removed = removeAll(arr, { a: 1 });
		expect(removed).toEqual([]);
	...
	it("does not re-insert if removed item is undefined", () => {
		...
		const removed = removeAll(arr, undefined);
	...
	it("removes only exact matches (NaN)", () => {
		...
		// NaN !== NaN, so remove will not remove any NaN
		const removed = removeAll(arr, NaN);
		expect(removed).toEqual([]);
