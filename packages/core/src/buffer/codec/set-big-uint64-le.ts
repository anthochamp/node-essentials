...
/**
 * Writes `value` into `target` at `offset` as a little-endian unsigned 64-bit
 * integer.
 *
 ...
 */
export function setBigUint64Le(
	target: Uint8Array,
	offset: number,
	value: bigint,
): void {
	setUint32Le(target, offset, Number(value & MASK_32N));
	setUint32Le(target, offset + 4, Number((value >> 32n) & MASK_32N));
}
