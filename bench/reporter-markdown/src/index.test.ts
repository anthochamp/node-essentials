...
import markdownReporter from "./index.js";
...
describe("markdownReporter", () => {
	it("renders one table per measure present in the run", async () => {
		...
		await render_(markdownReporter().createSink(memoryContext_(written)));

		const rendered = written.join("");
		expect(rendered).toContain("| fast |");
		expect(rendered).toContain("| steady |");
		// Two tables, so two header rows.
		expect(rendered.match(/\| case \|/g) ?? []).toHaveLength(2);
	});

	it("is selected by the id 'markdown'", () => {
		expect(markdownReporter().id).toBe("markdown");
	});
	...
	it("writes to the run's output base name unless given a path", () => {
		...
		markdownReporter().createSink(context);
		markdownReporter({ output: "docs/bench.md" }).createSink(context);

		expect(names).toEqual(["bench-results.md", "docs/bench.md"]);
	});
});
